...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--sentence", default=DEFAULT_SENTENCE, help="Dummy input sentence."
    )
    parser.add_argument(
        "--tile-size",
        type=int,
        ...
        matrices.append(
            {
                ...
                "tiles": tile_count,
                "tiling_efficiency": (
                    used_cells / provisioned_cells if provisioned_cells else 0.0
                ),
            }
        )
    return matrices
...
def build_fvcore_inputs(batch: Mapping[str, torch.Tensor]) -> tuple[torch.Tensor, ...]:
    ...
    return tuple(
        batch[key] for key in ordered_keys if key in batch and batch[key] is not None
    )
...
def collect_hook_metrics(
    ...
    def attention_hook(module_name: str, module: nn.Module):
        def hook(_: nn.Module, inputs: tuple[Any, ...], __: Any) -> None:
            input_tensors = [
                tensor for tensor in iter_tensors(inputs) if tensor.dim() == 3
            ]
    ...
    for module_name, module in model.named_modules():
        if module_name and len(list(module.children())) == 0:
            handles.append(
                module.register_forward_hook(activation_memory_hook(module_name))
            )
        if isinstance(module, nn.Linear):
            handles.append(
                module.register_forward_hook(linear_hook(module_name, module))
            )
        ...
        if is_attention_matrix_module(module):
            handles.append(
                module.register_forward_hook(attention_hook(module_name, module))
            )
    ...
    activation_sparsity = (
        activation_zero_count / activation_value_count
        if activation_value_count
        else 0.0
    )
...
def analyze_flops(
    model: nn.Module, fvcore_inputs: tuple[torch.Tensor, ...]
) -> dict[str, Any]:
...
def print_report(
    ...
    static_plus_dynamic = (
        flop_metrics["static_weight_macs"] + flop_metrics["dynamic_macs"]
    )
    dynamic_share = (
        flop_metrics["dynamic_macs"] / static_plus_dynamic
        if static_plus_dynamic
        else 0.0
    )
    static_share = (
        flop_metrics["static_weight_macs"] / static_plus_dynamic
        if static_plus_dynamic
        else 0.0
    )
    ...
    print(
        f"   Total crossbar tiles       : {format_int(tiling_metrics['total_tiles'])}"
    )
    print(
        f"   Provisioned cells          : {format_int(tiling_metrics['provisioned_cells'])}"
    )
    print(f"   Used weight cells          : {format_int(tiling_metrics['used_cells'])}")
    print(
        f"   Wasted padded cells        : {format_int(tiling_metrics['wasted_cells'])}"
    )
    print(
        f"   Tiling efficiency          : {format_percent(tiling_metrics['efficiency'])}"
    )
    print("-" * 92)
    print("4. Linear vs. Non-Linear Operation Ratio")
    print(f"   Linear/Dense MACs          : {format_int(flop_metrics['linear_macs'])}")
    print(
        f"   Non-linear/digital MACs    : {format_int(flop_metrics['nonlinear_macs'])}"
    )
    ...
    print(
        f"   Static-weight MACs         : {format_int(flop_metrics['static_weight_macs'])}"
    )
    ...
    print(
        "   These matrices are fixed after training and are natural AIMC crossbar candidates."
    )
    ...
    print(
        "   These matrices are formed at runtime from activations, so they require digital support."
    )
    ...
    if unsupported_ops:
        ignored = ", ".join(
            f"{name} ({count})" for name, count in unsupported_ops.items()
        )
        print("-" * 92)
